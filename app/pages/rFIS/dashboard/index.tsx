...
export default function Index(props:any){
    ...
    useEffect(()=>{
        dispatch(getRToken(Rtoken.rFis,Cycle.week));
    },[]);
    ...
    return <div>
          <DataRow>
          <DataCol title={"STAKED rFIS VALUE"} unit="$" amount={data?data.info.stakeValue:"--"} toolTip={"test"}/>
            ...
            <DataCol title={"rFIS/FIS"} amount={data?data.info.rate:"--"} toolTip={"test"}/>
            <DataCol title={"rFIS PRICE"} unit="$" amount={data?data.info.price:"--"} toolTip={"test"}/>
            <DataCol title={"rFIS AMOUNT"} unit="$" amount={data?data.info.rtokenAmount:"--"} toolTip={"test"}/>
            <DataCol title={"rFIS AMOUNT (ERC20)"} unit="$" amount={data?data.info.erc20RtokenAmount:"--"} toolTip={"test"}/>
        ...
        <DataRow>
            <ChartCol data={data?data.list.rate.data:[]} xData={data?data.list.rate.yData:[]} title="rFIS : FIS  RATE" barColor={"#40CB92"}/>
            <ChartCol data={data?data.list.price.data:[]} xData={data?data.list.price.yData:[]} title="rFIS PRICE" barColor={"#ED3C9A"}/>
            <ChartCol data={data?data.list.mint.data:[]} xData={data?data.list.mint.yData:[]} title="MINTED rFIS (NATIVE)" barColor={"#40CB92"}/>
            <ChartCol data={data?data.list.erc20Mint.data:[]} xData={data?data.list.erc20Mint.yData:[]} title="MINTED rFIS (ERC20)" barColor={"#ED3C9A"}/>
            <ChartCol data={data?data.list.stakeValue.data:[]} xData={data?data.list.stakeValue.yData:[]} title="STAKED rFIS VALUE IN STAFI PER WEEK" barColor={"#40CB92"}/>
            <ChartCol data={data?data.list.reddemValue.data:[]} xData={data?data.list.reddemValue.yData:[]} title="REDEEM rFIS VALUE IN STAFI PER WEEK" barColor={"#ED3C9A"}/>
            <ChartCol data={data?data.list.rtokenValue.data:[]} xData={data?data.list.rtokenValue.yData:[]} title="rFIS VALUE IN STAFI PER WEEK" barColor={"#40CB92"}/>
